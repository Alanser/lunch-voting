...
import com.alputov.lunchvoting.util.ValidationUtil;
import com.alputov.lunchvoting.util.exception.ApplicationException;
import com.alputov.lunchvoting.util.exception.ErrorInfo;
import com.alputov.lunchvoting.util.exception.ErrorType;
import com.alputov.lunchvoting.util.exception.IllegalRequestDataException;
...
import static com.alputov.lunchvoting.util.exception.ErrorType.*;
...
public class ExceptionInfoHandler {
    ...
    private static final Map<String, String> CONSTRAINS_MAP = Map.of(
            ...
            "restaurant_unique_name_idx", EXCEPTION_DUPLICATE_NAME_RESTAURANT_SAME_OWNER);

    @ExceptionHandler(ApplicationException.class)
    public ResponseEntity<ErrorInfo> applicationError(HttpServletRequest req, ApplicationException appEx) {
        ErrorInfo errorInfo = logAndGetErrorInfo(req, appEx, false, appEx.getType(), messageSourceAccessor.getMessage(appEx.getMsgCode(), appEx.getArgs()));
    ...
    public ErrorInfo conflict(HttpServletRequest req, DataIntegrityViolationException e) {
        ...
        if (rootMsg != null) {
            ...
            Optional<Map.Entry<String, String>> entry = CONSTRAINS_MAP.entrySet().stream()
                    ...
                    .findAny();
            if (entry.isPresent()) {
                return logAndGetErrorInfo(req, e, false, VALIDATION_ERROR, messageSourceAccessor.getMessage(entry.get().getValue()));
            }
        }
    ...
    public ErrorInfo bindValidationError(HttpServletRequest req, Exception e) {
        ...
        String[] details = result.getFieldErrors().stream()
                .map(fe -> messageSourceAccessor.getMessage(fe))
                .toArray(String[]::new);
